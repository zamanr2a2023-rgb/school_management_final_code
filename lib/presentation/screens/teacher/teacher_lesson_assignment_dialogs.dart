...
void showTeacherCreateLessonDialog(
  ...
  VoidCallback? onSuccess,
    ...
    if (cls != null) {
      final lv = cls.level.replaceAll(RegExp(r'\s*Grade\s*', caseSensitive: false), '').trim();
    ...
    showDialog(
      ...
      builder: (dialogContext) {
        ...
        final useApiCreate = editing == null && cls != null && lessonsRemote.isConfigured && gidFromClass.isNotEmpty && sidFromClass.isNotEmpty;
        final useApiEdit = editing != null && cls != null && lessonsRemote.isConfigured && gidFromClass.isNotEmpty && sidFromClass.isNotEmpty;
        final apiConfiguredMissingIds = editing == null && cls != null && lessonsRemote.isConfigured && (gidFromClass.isEmpty || sidFromClass.isEmpty);
        final apiEditMissingIds = editing != null && cls != null && lessonsRemote.isConfigured && (gidFromClass.isEmpty || sidFromClass.isEmpty);
        ...
        builder: (ctx, setDialogState) {
          Future<void> submitLesson(LessonStatus status) async {
            ...
            if (editing != null) {
              if (title.trim().isEmpty || module.trim().isEmpty) {
                messenger.showSnackBar(const SnackBar(content: Text('Title and chapter are required.')));
                return;
              }
              ...
              if (remote.isConfigured) {
                if (gid.isEmpty || sid.isEmpty) {
                  messenger.showSnackBar(const SnackBar(content: Text('This class has no gradeId or subjectId from the server. Reload class details while online, then try again.')));
                  return;
                }
                if (classIdForApi.isEmpty) {
                  messenger.showSnackBar(const SnackBar(content: Text('Missing class id.')));
                  return;
                }
              }

              final useApi = remote.isConfigured && gid.isNotEmpty && sid.isNotEmpty && classIdForApi.isNotEmpty;
              final apiStatus = status == LessonStatus.published ? 'published' : 'draft';
              final needsFile = type == 'pdf' || type == 'video';
              final hasNewFile = attachedFilePath != null && attachedFilePath!.isNotEmpty;
              final hasExistingContent = editing.content.trim().isNotEmpty;

              if (useApi && needsFile && !hasNewFile && !hasExistingContent) {
                messenger.showSnackBar(const SnackBar(content: Text('Attach a file for PDF or video lessons.')));
                return;
              }
              ...
              try {
                if (useApi) {
                  ...
                  if (ok) {
                    ...
                    messenger.showSnackBar(const SnackBar(content: Text('Lesson updated.')));
                  } else {
                    messenger.showSnackBar(const SnackBar(content: Text('Could not update lesson. Try again.')));
                  }
                  return;
                }

                hostContext.read<LessonsRepository>().updateLesson(editing.id, LessonEntity(
                  id: editing.id,
            ...
            final useApi = remote.isConfigured && gid.isNotEmpty && sid.isNotEmpty;
            final apiStatus = status == LessonStatus.published ? 'published' : 'draft';

            final needsFile = type == 'pdf' || type == 'video';
            if (useApi && needsFile && (attachedFilePath == null || attachedFilePath!.isEmpty)) {
              messenger.showSnackBar(const SnackBar(content: Text('Attach a file for PDF or video lessons.')));
              return;
            }
            ...
            try {
              if (useApi) {
                ...
                if (ok) {
                  onSuccess?.call();
                  if (ctx.mounted) Navigator.pop(ctx);
                  messenger.showSnackBar(SnackBar(content: Text(apiStatus == 'published' ? 'Lesson published.' : 'Lesson saved as draft.')));
                } else {
                  messenger.showSnackBar(const SnackBar(content: Text('Could not create lesson. Check network, file, and try again.')));
                }
                return;
              }
          ...
          final maxDialogHeight =
              (mq.size.height - keyboardInset - mq.padding.vertical - 48).clamp(220.0, mq.size.height);
                  ...
                  child: Column(
                    ...
                    Row(
                      children: [
                        Expanded(
                          child: Text(
                            editing != null ? _tr(lang, 'teacherClassDetails.editLesson', 'Edit Lesson') : _tr(lang, 'teacherClassDetails.createLesson', 'Create New Lesson'),
                    ...
                    const SizedBox(height: 12),
                          dialogLabel(_tr(lang, 'teacherClassDetails.lessonTitle', 'Lesson Title') + ' *'),
                          ...
                          const SizedBox(height: 12),
                          dialogLabel(_tr(lang, 'lessons.description', 'Description')),
                          ...
                          const SizedBox(height: 12),
                          dialogLabel(_tr(lang, 'teacherClassDetails.contentType', 'Content Type') + ' *'),
                          ...
                          const SizedBox(height: 12),
                          dialogLabel(_tr(lang, 'teacherClassDetails.chapter', 'Chapter') + ' *'),
                          ...
                          if (cls != null && (useApiCreate || useApiEdit)) ...[
                            dialogLabel(_tr(lang, 'teacherClassDetails.classFromServer', 'Class')),
                            Container(
                              width: double.infinity,
                              ...
                              child: Column(
                                ...
                                children: [
                                  Text(
                                    ...
                                    style: const TextStyle(fontWeight: FontWeight.w600, color: labelColor, fontSize: 14),
                                  ),
                              ...
                              children: [
                                Expanded(
                                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                    dialogLabel(_tr(lang, 'live.grade', 'Grade') + ' *'),
                                    DropdownButtonFormField<String>(
                                ...
                                Expanded(
                                  child: Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
                                    dialogLabel(_tr(lang, 'live.subject', 'Subject') + ' *'),
                                    DropdownButtonFormField<String>(
                                      value: subject,
                                      decoration: inputDecoration('').copyWith(contentPadding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10)),
                                      isExpanded: true,
                          ...
                          const SizedBox(height: 12),
                          dialogLabel(_tr(lang, 'teacherClassDetails.attachFiles', 'Attach Files')),
                          Text(
                            editing != null
                          ...
                          GestureDetector(
                            onTap: () async {
                              final r = await FilePicker.platform.pickFiles(
                                ...
                                allowedExtensions: const ['pdf', 'mp4', 'mov', 'webm', 'png', 'jpg', 'jpeg'],
                              );
                    ...
                    Wrap(
                      ...
                      children: [
                        OutlinedButton(
                          onPressed: submitting[0] ? null : () => Navigator.pop(ctx),
                          ...
                          child: Text(_tr(lang, 'common.cancel', 'Cancel'), style: const TextStyle(fontSize: 13)),
                        ),
                        if (editing != null)
                          FilledButton(
                            onPressed: (submitting[0] || apiEditMissingIds) ? null : () => submitLesson(editing.status),
                            ...
                            child: submitting[0]
                                ...
                                : Text(_tr(lang, 'teacherClassDetails.updateLesson', 'Update Lesson'), style: const TextStyle(fontSize: 13)),
                          )
                        else ...[
                          OutlinedButton(
                            onPressed: (submitting[0] || apiConfiguredMissingIds) ? null : () => submitLesson(LessonStatus.draft),
                            ...
                            child: Text(_tr(lang, 'teacherClassDetails.saveAsDraft', 'Save as Draft'), style: const TextStyle(fontSize: 12)),
                          ),
                          FilledButton(
                            onPressed: (submitting[0] || apiConfiguredMissingIds) ? null : () => submitLesson(LessonStatus.published),
                            style: FilledButton.styleFrom(backgroundColor: labelColor, padding: const EdgeInsets.symmetric(horizontal: 12, vertical: 10)),
                            child: submitting[0]
                                ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
                                : Text(
                                    useApiCreate ? _tr(lang, 'teacherClassDetails.publishLesson', 'Publish') : _tr(lang, 'teacherClassDetails.createLesson', 'Create Lesson'),
    ...
    showDialog(
      ...
      builder: (ctx) {
        ...
        builder: (ctx, setDialogState) {
          Future<void> submit() async {
            ...
            if (editing != null) {
              if (title.trim().isEmpty || dueDate.trim().isEmpty) {
                messenger.showSnackBar(const SnackBar(content: Text('Title and due date are required.')));
                return;
              }
              ...
              if (remote.isConfigured) {
                ...
                setDialogState(() {});
                try {
                  final timeStr = dueTime.trim().isEmpty ? '23:59' : dueTime.trim();
                  final gid = cls?.gradeId?.trim() ?? '';
                  final sid = cls?.subjectId?.trim() ?? '';
                  ...
                  if (ok) {
                    ...
                    if (ctx.mounted) Navigator.pop(ctx);
                    messenger.showSnackBar(const SnackBar(content: Text('Assignment updated.')));
                  } else {
                    messenger.showSnackBar(const SnackBar(content: Text('Could not update assignment. Check fields and network.')));
                  }
                } finally {
                  submitting[0] = false;
                  if (ctx.mounted) setDialogState(() {});
            ...
            if (remote.isConfigured && (gid.isEmpty || sid.isEmpty)) {
              messenger.showSnackBar(const SnackBar(
                content: Text('This class has no gradeId or subjectId from the server. Reload class details, then try again.'),
              ));
              return;
            }
            ...
            try {
              if (useApi) {
                ...
                if (ok) {
                  ...
                  if (ctx.mounted) Navigator.pop(ctx);
                  messenger.showSnackBar(const SnackBar(content: Text('Assignment created.')));
                } else {
                  messenger.showSnackBar(const SnackBar(content: Text('Could not create assignment. Check fields and network.')));
                }
                return;
              }
              ...
              if (onAppendAssignmentLocal != null) {
                ...
                hostContext.read<AssignmentsRepository>().addAssignment(newAssign);
              }
              if (ctx.mounted) Navigator.pop(ctx);
            } finally {
              submitting[0] = false;
          ...
          final mq = MediaQuery.of(ctx);
          final maxDialogHeight =
              (mq.size.height - mq.viewInsets.bottom - mq.padding.vertical - 48).clamp(220.0, mq.size.height);
                ...
                child: Padding(
                  ...
                  Row(
                    children: [
                      Expanded(
                        child: Text(
                          editing != null ? _tr(lang, 'teacherClassDetails.editAssignment', 'Edit Assignment') : _tr(lang, 'teacherClassDetails.createAssignment', 'Create New Assignment'),
                          style: const TextStyle(fontSize: 18, fontWeight: FontWeight.bold, color: labelColor),
                        ),
                      ),
                      IconButton(
                  ...
                  const SizedBox(height: 16),
                          dialogLabel(_tr(lang, 'teacherClassDetails.assignmentTitle', 'Assignment Title'), required: true),
                          ...
                          const SizedBox(height: 12),
                          dialogLabel(_tr(lang, 'assignments.dueDate', 'Due Date'), required: true),
                          TextField(
                          ...
                          const SizedBox(height: 12),
                          dialogLabel(_tr(lang, 'teacherClassDetails.dueTime', 'Due Time')),
                          TextField(
                          ...
                          const SizedBox(height: 12),
                          dialogLabel(_tr(lang, 'assignments.points', 'Points'), required: true),
                  ...
                  Wrap(
                    ...
                    children: [
                      ...
                      FilledButton(
                        onPressed: (submitting[0] || (editing == null && apiCreateMissingIds)) ? null : submit,
                        style: FilledButton.styleFrom(backgroundColor: labelColor, padding: const EdgeInsets.symmetric(horizontal: 14, vertical: 10)),
                        child: submitting[0]
                            ? const SizedBox(width: 18, height: 18, child: CircularProgressIndicator(strokeWidth: 2, color: Colors.white))
                            : Text(editing != null ? _tr(lang, 'common.update', 'Update') : _tr(lang, 'teacherClassDetails.createAssignment', 'Create Assignment'), style: const TextStyle(fontSize: 13)),
                      ),
                    ],
                  ),
                ],
